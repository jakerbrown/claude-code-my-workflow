# Forking a Claude Workflow Into Codex: What Changed, What Stayed the Same, and What It Takes to Port It
...
> The simplest way to think about a workflow repo is as a toolbox plus a
> rulebook for working with an AI assistant. Instead of opening a blank chat
> and hoping for the best, you give the assistant structure:
>
...
1. Plan first. This is pretty standard operating procedure with Codex or
   Claude ("enter plan mode" or "make a plan").
2. Keep durable notes on disk so context is not trapped in one chat window.
3. Use specialists when one general reviewer is not enough.
4. Verify outputs instead of trusting fluent prose.
5. Gate completion with explicit quality thresholds.
6. Use an adversarial critic/fixer loop instead of a single pass.
...
> Claude and Codex are similar in the way two kitchens are similar: both have
> heat, knives, and counters, but the appliances are in different places and
> some of them work differently. A recipe can survive that move, but not by
> copying the layout blindly.
